...
const MusicPlayer = () => {
  ...
  const renderSongs = useCallback((songs) => {
    return songs.map((song) => (
      <li key={song.id} id={`song-${song.id}`} className="playlist-song" onClick={() => playSong(song.id)}>
        ...
        <button onClick={(e) => { e.stopPropagation(); deleteSong(song.id); }} className="playlist-song-delete" aria-label={`Delete ${song.title}`}>
